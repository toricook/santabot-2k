...
import { ProfileWishlistForm } from "@/app/profile/ProfileWishlistForm";
...
export default async function ProfileCatchAllPage({ params }: ProfileCatchAllPageProps) {
  ...
  const firstSegment = params.rest?.[0]?.toLowerCase();
  ...
  if (firstSegment === "account") {
    const user = await requireSessionUser(userId);
        ...
        <div className="mx-auto max-w-4xl rounded-3xl border border-green-100 bg-white/90 p-8 shadow-xl shadow-red-100/40">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-green-100 pb-5">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-bold text-red-600">Account & wishlist</h1>
              <p className="text-sm text-green-800">
                Manage your Clerk account details and keep your wishlist current so your Santa knows
                what to wrap.
              </p>
            </div>
            <Link
          ...
          <div className="mt-8 space-y-8">
            <section className="rounded-3xl border border-green-100 bg-white/90 p-6 shadow-inner shadow-red-50">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.5em] text-green-600">
                    Account overview
                  </p>
                  <h2 className="mt-1 text-2xl font-bold text-red-600">{user.name}</h2>
                  <p className="text-sm text-green-800">{user.email}</p>
                </div>
                <Link
                  href="https://dashboard.clerk.com/u"
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center rounded-full border border-green-700 px-5 py-2 text-sm font-semibold text-green-800 transition hover:border-red-500 hover:text-red-600"
                >
                  Manage account on Clerk
                </Link>
              </div>
              <p className="mt-4 text-sm text-green-700">
                Use the button above to update login details, connected accounts, or security
                preferences directly in Clerk. Your wishlist lives below for quick edits.
              </p>
            </section>

            <ProfileWishlistForm initialWishlist={user.wishlist ?? ""} />
          </div>
        </div>
      </div>
    );
